...
def get_relation(avg_grad, idxs_users):   #好像沒除idex_users
...
def probabilistic_selection(node_prob, node_count, act_indx, part_node_after, labeled, alpha):
    ...
    remove_list = Diff(act_indx, part_node_after) #這輪中expectation和loss過不了的
    ...
    #dict.keys():以列表返回一个字典所有的keys
    rest_nodes = Diff(list(node_prob.keys()), labeled) #除了被標記的以外的所有user(包括這輪沒被選到的)
    beta = 0.7
    weight = 0
    ...
    #从 node_prob 字典中根据概率选择 10 个不重复的键，并将结果存储在 get_node 变量中。
