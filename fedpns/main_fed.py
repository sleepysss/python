...
algorithm:
fedavg
fedbn2:Convergence of Update Aware Device Scheduling for Federated Learning at the Wireless Edge提的
fedsel:即fedpns 我猜fedpns=fedsel=feddel
...
if __name__ == '__main__':
    ...
    loss_all = {}
    acc_all = {}
    for exp in tqdm(range(1, args.num_exp+1)):  #epochs
        ...
        node_prob = {}
        # node_count = {}
        test_count = {} #key:all users' index ;value:list of three elements(猜測:第一個:被選到的次數,第二個:被標記的次數,第三個我找不到相關的)
        # all_user = list(range(args.num_users))
            # participate_count = {}
        for i in range(args.num_users):
            node_prob[i] = 1 / args.num_users
            # node_count[i] = 0 
            tupe = [] #一个包含三个元素的列表，初始值都为0
            for j in range(3):
                tupe.append(0)
                test_count[i] = tupe
        ...
        for iter in range(1, args.rounds+1): #應該是user epochs
            ...
            if args.algorithm == "fedavg":
                ...
                logger.info('user %s', sorted(idxs_users))
            elif iter == 1:  #第一次for前面都不符合一定會到這裡(fedpns第一輪會到這,之後每輪fedpns都是按照機率選10個users)
                #不確定為啥要選兩次
                idxs_users = random.sample(range(int(args.num_users*args.ratio)), 5) #第一次隨機抽樣
                other = random.sample(range(int(args.num_users*args.ratio), 50), 5 ) #第二次隨機抽樣
                idxs_users.extend(other)
            ...
            for idx in idxs_users: #each user
                ...
                gradient[idx] = copy.deepcopy(g) #存user的gradient
            ...
            if args.algorithm == "fedsel": #fedpns
                
                #gradient多加一組key-value pair
                gradient['avg_grad'] = average(gradient)
                max_now = get_relation(gradient, idxs_users) #algo1的max(但他的get_relation(也就是算expectation)怪怪的)
                
                #idxs_before: 這輪一開始選的(完全沒刪)
                #idxs_left: 移除幾個clients(expectation和loss都過不了的)後剩的
                #labeled:此輪中expectation過不了的
                #w_locals:weight of local models(是個dict) 返回的是已經刪除expectation和loss的沒過的users
                #test_count:
