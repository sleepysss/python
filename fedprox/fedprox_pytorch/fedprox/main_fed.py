...
if __name__ == '__main__': #自己執行時才會做if下面的動作,被別人引用時則不會執行
    ...
    for iter in range(args.epochs): #幾輪
        ...
        exists_users = np.random.choice(idxs_users,int(m*(1-args.drop)),replace=False) 
        
        for idx in idxs_users: #each user
            args.local_ep=local_ep
            if idx in exists_users:
                local = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx])
                w, loss = local.train(net=copy.deepcopy(net_glob).to(args.device)) #net=... 拿global model
            else:
                if local_ep==1:  #1的話會變成1<=輸出<1
                    args.local_ep=1
                else:
                    args.local_ep=random.randrange(1, local_ep)  #1<=輸出<local_ep, drop的user能完成幾輪
                local = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx])
                w, loss = local.train(net=copy.deepcopy(net_glob).to(args.device)) #net=... 拿global model 
            if args.all_clients:
